...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!booking_id) throw new Error("Falta el número de reserva (booking_id).");
    ...
    if (bookingError || !booking) throw new Error("La reserva no existe.");

    if (user.id !== booking.host_id && user.id !== booking.renter_id) {
      throw new Error("No tienes permiso para leer este chat.");
    }
    ...
    if (messagesError) throw new Error(`Error al cargar los mensajes: ${messagesError.message}`);
